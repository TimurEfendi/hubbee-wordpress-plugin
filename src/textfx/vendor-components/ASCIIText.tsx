...
// Original source: reactbits.dev - ASCIIText
...
function mathMap(n: number, start: number, stop: number, start2: number, stop2: number): number {
...
class AsciiFilter {
  ...
  reset() {
    // Set font on pre first, then measure actual DOM char width
  ...
  render(scene: THREE.Scene, camera: THREE.PerspectiveCamera) {
    ...
    // After first render: scale pre to fill container width exactly
    if (!this._preScaled && this.pre.textContent) {
      const naturalW = this.pre.scrollWidth;
      if (naturalW > 0 && naturalW !== this.width) {
        this.pre.style.transform = `scaleX(${this.width / naturalW})`;
        this.pre.style.transformOrigin = 'left top';
      }
      this._preScaled = true;
    }
  }
...
class CanvAscii {
  ...
  constructor(
    ...
    this.camera.position.z = 30; // default, recalculated in setMesh()
  ...
  async init() {
    // Inject font stylesheet into document head BEFORE font.load
    if (!document.querySelector('#hubbee-ibm-plex-mono')) {
      const link = document.createElement('link');
      link.id = 'hubbee-ibm-plex-mono';
      link.rel = 'stylesheet';
    ...
    // Safety net: if font loads AFTER init, re-measure everything
    document.fonts.addEventListener('loadingdone', () => {
      if (!this.textCanvas || !this.texture || !this.filter) return;
  ...
  setMesh() {
    ...
    // Calculate visible area at z=0 for camera at z=30
    const cameraZ = 30;
    const fovRad = (45 / 2) * Math.PI / 180;
    const containerAspect = (this.width / this.height) || 1;
    const visibleH = 2 * cameraZ * Math.tan(fovRad);
    const visibleW = visibleH * containerAspect;

    // Scale plane to fit within visible area (90% to leave margin for wave animation)
    const textAspect = this.textCanvas.width / this.textCanvas.height;
    const fitByWidth = visibleW * 0.9;
    const fitByHeight = visibleH * 0.9;
    let planeW = fitByWidth;
    let planeH = planeW / textAspect;
    if (planeH > fitByHeight) {
      planeH = fitByHeight;
...
export default function ASCIIText({
      ...
      if (width === 0 || height === 0) {
        observer = new IntersectionObserver(async ([entry]) => {
          ...
          if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
            ...
            if (!cancelled) {
              asciiRef.current = await createAndInit(containerRef.current!, entry.boundingClientRect.width, entry.boundingClientRect.height);
  ...
  return (
    <div ref={containerRef} style={{ position: 'absolute', width: '100%', height: '100%', overflow: 'hidden' }}>
      <style>{`
        div > div canvas { position: absolute; left: 0; top: 0; width: 100%; height: 100%; image-rendering: pixelated; }
        div > div pre { margin: 0; user-select: none; padding: 0; line-height: 1em; text-align: left; position: absolute; left: 0; top: 0; overflow: hidden; background-image: radial-gradient(circle, #ff6188 0%, #fc9867 50%, #ffd866 100%); background-attachment: fixed; -webkit-text-fill-color: transparent; -webkit-background-clip: text; z-index: 9; mix-blend-mode: difference; }
      `}</style>
    </div>
  );
}
