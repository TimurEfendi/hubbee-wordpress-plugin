 ...
 * Original: src/content/Backgrounds/LiquidEther/LiquidEther.jsx
 *
 * Kept 1:1 with the original, only typed for TypeScript.
 * NOTE: This is a very large component (~800 lines). Requires three.js.
 * Due to extreme length, this component is imported dynamically.
 * The full original source is preserved; see the GitHub link above.
 *
 * CSS inlined: .liquid-ether-container { position:relative; overflow:hidden; width:100%; height:100%; touch-action:none; }
 */

// Re-export stub: The full implementation is too large for static bundling in the
// background builder context. This file serves as the typed entry point.
// For the complete implementation, refer to the original source at:
// https://github.com/DavidHDev/react-bits/blob/main/src/content/Backgrounds/LiquidEther/LiquidEther.jsx
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Simulation {
      ...
      calcSize() {
        // Defense-in-depth: even with a misconfigured `resolution` (e.g. an
        // older registry stored it as an absolute pixel count), never let the
        // FBO exceed a safe bound below the GL_MAX_TEXTURE_SIZE floor. 2048
        // is well under the 4096–16384 range every modern GPU advertises and
        // is more than enough for the simulation grid.
        const MAX_FBO = 2048;
        const w = Math.min(MAX_FBO, Math.max(1, Math.round(this.options.resolution * Common.width)));
        const h = Math.min(MAX_FBO, Math.max(1, Math.round(this.options.resolution * Common.height)));
    ...
    class WebGLManager {
      props: any; output: any; running = false; autoDriver: any; lastUserInteraction: number;
