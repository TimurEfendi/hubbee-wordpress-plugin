 ...
 * Original: src/content/Backgrounds/LightPillar/LightPillar.jsx
 *
 * Kept 1:1 with the original, only typed for TypeScript.
 * NOTE: Requires three.js. CSS inlined as inline styles.
 */
...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (rendererRef.current) {
        rendererRef.current.domElement.removeEventListener('webglcontextlost', onContextLost);
  ...
  if (!webGLSupported) {
    return (
      <div className={`light-pillar-fallback ${className}`} style={{
        ...
        backgroundColor: 'rgba(0,0,0,0.1)', color: '#888', fontSize: '14px',
  ...
  return <div ref={containerRef} className={`light-pillar-container ${className}`} style={{
    width: '100%', height: '100%', position: 'absolute', top: 0, left: 0,
    mixBlendMode: mixBlendMode as any
  }} />;
};
