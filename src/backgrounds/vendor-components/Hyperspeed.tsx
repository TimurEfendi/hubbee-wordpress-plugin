 ...
 * Original: src/content/Backgrounds/Hyperspeed/Hyperspeed.jsx
 *
 * Kept 1:1 with the original, only typed for TypeScript.
 * NOTE: Requires three.js and postprocessing as dependencies.
 * CSS from Hyperspeed.css inlined below.
 */
...
/* Inlined CSS (originally Hyperspeed.css):
#lights {
  width: 100%;
  height: 100%;
  overflow: hidden;
  ...
  useEffect(() => {
    ...
    class App {
      ...
      constructor(container: any, options: any = {}) {
        ...
        // Defensive: keep the tab alive if the browser kills the GL context
        // (e.g. memory pressure). The animation loop simply pauses; the next
        // configurator interaction triggers a clean unmount + remount.
      ...
      initPasses() {
        // Each pass is constructed defensively: a thrown EffectPass (e.g.
        // from a postprocessing/three version drift) must not leak null into
        // composer.addPass, which would crash the entire renderer.
        try {
          this.renderPass = new RenderPass(this.scene, this.camera);
        } catch (err) { console.error('[Hyperspeed] RenderPass init failed', err); this.renderPass = null; }
        try {
        ...
        } catch (err) { console.error('[Hyperspeed] BloomPass init failed', err); this.bloomPass = null; }
        ...
        } catch (err) { console.error('[Hyperspeed] SMAAPass init failed', err); }
        ...
        // `addPass` itself can crash if the composer's renderer/context is null
        // (postprocessing version drift, dev-mode StrictMode double-mount).
        // Each call is isolated so one failing pass cannot abort the others.
        if (this.renderPass) {
          try { this.composer.addPass(this.renderPass); }
          catch (err) { console.error('[Hyperspeed] addPass renderPass failed', err); }
        }
        if (this.bloomPass) {
          try { this.composer.addPass(this.bloomPass); }
          catch (err) { console.error('[Hyperspeed] addPass bloomPass failed', err); }
        }
        if (smaaPass) {
          try { this.composer.addPass(smaaPass); }
          catch (err) { console.error('[Hyperspeed] addPass smaaPass failed', err); }
        }
      }
      ...
      dispose() {
        ...
        if (this.renderer) {
          if (this._onContextLost) this.renderer.domElement.removeEventListener('webglcontextlost', this._onContextLost);
