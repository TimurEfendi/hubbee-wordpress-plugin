 ...
 * Original: src/content/Backgrounds/Orb/Orb.jsx
 *
...
export default function Orb(props: OrbProps) {
  ...
  const frag = /* glsl */ `
    precision highp float;
    uniform float iTime; uniform vec3 iResolution; uniform float hue; uniform float hover; uniform float rot; uniform float hoverIntensity; uniform vec3 backgroundColor;
    varying vec2 vUv;
    ...
    vec4 draw(vec2 uv){
      ...
      float n0=snoise3(vec3(uv*noiseScale,iTime*0.5))*0.5+0.5;float r0=mix(mix(innerRadius,1.0,0.4),mix(innerRadius,1.0,0.6),n0);float d0=distance(uv,(r0*invLen)*uv);float v0=light1(1.0,10.0,d0);
      v0*=smoothstep(r0*1.05,r0,len);float innerFade=smoothstep(r0*0.8,r0*0.95,len);v0*=mix(innerFade,1.0,bgLuminance*0.7);float cl=cos(ang+iTime*2.0)*0.5+0.5;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: { iTime: { value: 0 }, iResolution: { value: new Vec3(gl.canvas.width, gl.canvas.height, gl.canvas.width / gl.canvas.height) }, hue: { value: initial.hue ?? 0 }, hover: { value: 0 }, rot: { value: 0 }, hoverIntensity: { value: initial.hoverIntensity ?? 0.2 }, backgroundColor: { value: hexToVec3(initial.backgroundColor ?? '#000000') } }
    });
    ...
    let targetHover = 0; let lastTime = 0; let currentRot = 0; const rotationSpeed = 0.3;
    const handleMouseMove = (e: MouseEvent) => {
      const rect = container.getBoundingClientRect();
      const x = e.clientX - rect.left; const y = e.clientY - rect.top;
      const width = rect.width; const height = rect.height; const size = Math.min(width, height);
      const centerX = width / 2; const centerY = height / 2;
      const uvX = ((x - centerX) / size) * 2.0; const uvY = ((y - centerY) / size) * 2.0;
    ...
    const update = (t: number) => {
      ...
      program.uniforms.hue.value = live.hue ?? 0;
      program.uniforms.hoverIntensity.value = live.hoverIntensity ?? 0.2;
      const bg = live.backgroundColor ?? '#000000';
      if (bg !== lastBg) { lastBg = bg; program.uniforms.backgroundColor.value = hexToVec3(bg); }
      const effectiveHover = (live.forceHoverState ?? false) ? 1 : targetHover;
      program.uniforms.hover.value += (effectiveHover - program.uniforms.hover.value) * 0.1;
      if ((live.rotateOnHover ?? true) && effectiveHover > 0.5) currentRot += dt * rotationSpeed;
